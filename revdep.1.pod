...
L<ld.so(8)|https://www.mankier.com/8/ld.so> uses.
...
=item B<-R>, B<--revdepdir>=I<PATH>

Specify an alternate location for B<revdep> package configuration.

Default location is F<@SYSCONFDIR@/revdep.d>.
...
Upon startup, B<revdep> searches the F<@SYSCONFDIR@/revdep.d>
directory for files named after packages.  If any are found, any
directories (assumed to be absolute paths and delimited by newlines)
within the file are added to a list of package specific directories to
search if a library dependency is not found in one of the many standard
locations (as specified by L<ld.so(8)|https://www.mankier.com/8/ld.so>).
...
F<firefox> should be created under F<@SYSCONFDIR@/revdep.d> with a
...
C++ rewrite inspired by Johannes Winkelmann's L<jw@tks6.net>
shell B<revdep>.

Contributed to design Danny Rawlins L<contact@romster.me>.
